...
function* coinsWorker({ payload }) {
  yield put({
    type: coinActions.FETCH_COINS_REQUEST_LOADING,
  });
  try {
    const data = yield call(coinServices.fetchCoins, payload);
    yield put({
...
function* coinDetails({ payload }) {
  yield put({
    type: coinActions.FETCH_COIN_DETAILS_LOADING,
  });
  try {
    const coinData = yield call(coinServices.fetchCoinDetails, payload);
    yield put({
...
function* coinDetailsWorker({ payload }) {
...
function* coinChartDataWorker({ payload }) {
  yield put({
    type: coinActions.FETCH_COIN_CHART_DATA_LOADING,
  });
  try {
    const data = yield call(coinServices.fetchCoinChartData, payload);
    yield put({
...
const sagaWatchers = [coinsWatcher(), coinDetailsWatcher(), coinChartDataWatcher()];
