...
const linkWithToolTip = (link, index, isCopyable,) =>
      ...
      {Array.isArray(links) ? (
        links.map(
          (link, index) =>
            link &&
            link !== "" && (
              linkWithToolTip(link, index, true)
            )
        )
      ) : typeof links == "object" ? (
        Object.keys(links).map((linkTitle, index) => {
          return links[linkTitle].find((link) => link && link !== "") ? (
            <div key={`link-object-value-${index}`}>
              <span className="second-lvl-link-title">
                <i>{linkTitle}</i>
              </span>
              <div>
                {links[linkTitle].map((link, linkIndex) => (
                  linkWithToolTip(link, linkIndex, true)
                ))}
              </div>
            </div>
          ) : null;
...
const CoinDetails = () => {
  const { data, error, loading } = useSelector((state) => state.coin);
