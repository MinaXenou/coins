...
const renderLegend = (props, color, t) => {
  const { payload } = props;
  return (
    <>
...
const Chart = ({ data }) => {
  ...
  const pricesData = prices.map((i, index) => ({
    dt: moment(i[0]).format("DD/MM/YY HH:mm:ss"),
    price: i[1],
  }));
  const marketCapsData = market_caps.map((i, index) => ({
    dt: moment(i[0]).format("DD/MM/YY HH:mm:ss"),
    market_cap: i[1],
  }));
  const totalVolumesData = total_volumes.map((i, index) => ({
    dt: moment(i[0]).format("DD/MM/YY HH:mm:ss"),
    total_volume: i[1],
  }));
